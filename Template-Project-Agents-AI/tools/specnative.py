...
def parse_task_entries(text: str) -> list[dict[str, Any]]:
    ...
    for raw_line in text.splitlines():
        ...
        heading = re.match(r"^###\s+([A-Z0-9-]+)\s+-\s+(.+)$", line)
        if heading:
            if current:
        ...
        if current:
            ...
            if last_key and continuation and raw_line.startswith((" ", "\t")):
                current[last_key] = f"{current[last_key]} {continuation}".strip()
...
def validate() -> int:
    ...
    for task_path in find_task_files():
        ...
        for task in task_file["tasks"]:
            if task.get("estado") not in TASK_STATES and task.get("state") not in TASK_STATES:
                errors.append(f"{task_path.relative_to(ROOT)}: task {task.get('id')} missing valid state")
